...
int main(int argc, char** argv)
{
...
#endif
  std::optional<beat::Waveform> wd_rev_kernel = std::nullopt;
#ifdef USE_SYNTH_REVERB
  wd_rev_kernel = beat::WaveformIO::load(wk_dir + "s1_r1_o.wav", 3);
#endif
  std::vector<beat::Waveform> synth_waves;
  std::vector<beat::AudioSource*> synth_sources;
  for (int i = 0; i < static_cast<int>(beat::InstrumentType::NUM_ITEMS); ++i)
  {
    auto instrument = static_cast<beat::InstrumentType>(i);
    float duration = 2.f;
    if (i >= 6)
      duration = 0.2f;
    ...
    if (i == 6)
      ...
      frequency = 1200.f;
    auto wd_synth = beat::Synthesizer::synthesize(instrument, wave_gen, duration, frequency,
      44100, false,
      beat::FrequencyType::CONSTANT, beat::AmplitudeType::VIBRATO_0);
    if (wd_rev_kernel.has_value())
      wd_synth = beat::WaveformHelper::reverb_fast(wd_synth, wd_rev_kernel.value());
    //wd_synth = beat::WaveformHelper::fir_flange(wd_synth);
  ...
  for (int i = 0; i < static_cast<int>(beat::InstrumentType::NUM_ITEMS); ++i)
  {
    ...
    beat::Synthesizer::print_instrument(static_cast<beat::InstrumentType>(i));
    auto [min_val, max_val] = beat::WaveformHelper::find_min_max(wd_synth);
    std::cout << "(min, max) = (" << min_val << ", " << max_val << ")\n";
    for (int j = 0; j < (i >= 6 ? 3 : 1); ++j)
    {
      src_synth->play(beat::PlaybackMode::STATE_WAIT);
      src_synth->stop();
      Delay::sleep(100000);
    }
  }
